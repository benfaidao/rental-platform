      ...
      <div className="card space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative">
            ...
            <input className="input pl-9 w-48" placeholder="N° contrat, client..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          ...
          <div className="flex gap-1 items-center">
            {['', 'PENDING', 'ACTIVE', 'COMPLETED', 'CANCELLED'].map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium ${statusFilter === s ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                {s ? STATUS[s] : 'Tous'}
              </button>
            ))}
          </div>
