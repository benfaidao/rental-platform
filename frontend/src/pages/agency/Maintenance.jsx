  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => { setForm({ carId: '', date: '', mileage: '', oilType: '', filterChanged: false, cost: '', notes: '', nextKm: '', nextDate: '' }); setModal({ record: null }) }} className="btn-primary flex items-center gap-2">
      ...
      <Modal isOpen={!!modal} onClose={() => setModal(null)} title={modal?.record ? 'Modifier Vidange' : 'Nouvelle Vidange'}>
        <form onSubmit={submitForm} className="space-y-4">
          ...
          <div className="flex justify-end"><button type="submit" className="btn-primary" disabled={createMutation.isPending || updateMutation.isPending}>Enregistrer</button></div>
        </form>
      </Modal>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between">
        <select className="input w-56" value={carFilter} onChange={e => setCarFilter(e.target.value)}>
          <option value="">Tous les véhicules</option>
          {cars.map(c => <option key={c.id} value={c.id}>{c.brand} {c.model} — {c.finalPlate || c.wwPlate}</option>)}
        </select>
        <button onClick={() => { setForm({ carId: '', date: '', mileage: '', position: '', brand: '', size: '', cost: '', notes: '' }); setModal({ record: null }) }} className="btn-primary flex items-center gap-2">
...
function Repairs({ agencyId, cars }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <CarFilter cars={cars} value={carFilter} onChange={setCarFilter} />
        <button onClick={() => { setForm({ carId: '', date: '', description: '', mileage: '', cost: '', garage: '', nextRepairDate: '', nextRepairDescription: '', notes: '' }); setPhotoFiles([]); setModal({ record: null }) }} className="btn-primary flex items-center gap-2">
